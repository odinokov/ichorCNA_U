...
if (is.null(exons.bed) || exons.bed == "NULL"){
...
## load PoN
if (is.null(normal_panel) || normal_panel == "NULL"){
	normal_panel <- NULL
}

## load Blacklisted regions
if (is.null(centromere) || centromere == "NULL"){
	centromere <- NULL
}
...
for (i in 1:numSamples) {
	...
	dir.create(paste0(outDir, "/", id, "/"), recursive = TRUE, showWarnings = FALSE)
	...
	# BM Note: none of this works if there isnt a normal_file
	### DETERMINE GENDER ### 
	...
	write.table(tumour_copy[[id]], file=paste0(outDir, "/", patientID, "/",patientID,".rawCounts.tsv"), row.names=F, col.names=T, quote=F, sep="\t")
	...
	write.table(tumour_copy[[id]], file=paste0(outDir, "/", patientID, "/",patientID,".normalizedData.tsv"), row.names=F, col.names=T, quote=F, sep="\t")
	...
	for (p in ploidy){
		...
		for (s in 1:numSamples){
			...
			correctedResults <- correctIntegerCN(cn = hmmResults.cor$cna[[s]],
						...
						gender = "NONE", chrs = chrs, correctHOMD = includeHOMD)
