...
####################################
# updated for GRanges #
keepChr <- function(tumor_reads, chrs = c(1:22,"X","Y")){	
	tumor_reads <- tumor_reads[seqnames(tumor_reads) %in% chrs]
	sortSeqlevels(tumor_reads)
	return(sort(tumor_reads))
}
...
filterByTargetedSequences <- function(counts, targetedSequences){
 ### for targeted sequencing (e.g.  exome capture),
    ### ignore bins with 0 for both tumor and normal
...
normalizeByPanelOrMatchedNormal <- function(tumor_copy, chrs = c(1:22, "X", "Y"), 
	...
	chrXInd <- grep("X", as.character(seqnames(tumor_copy)))
	chrXMedian <- median(tumor_copy[chrXInd, ]$copy, na.rm = TRUE)
	...
	if (!is.null(normal_copy) && !is.null(normal_panel) && gender=="male"){
			...
			chrX.MNnorm <- tumor_copy$copy[chrXInd] - normal_copy$copy[chrXInd]
	...
	if (is.null(normal_copy) && gender=="male" && !gender.mismatch && normalizeMaleX){
			tumor_copy$copy[chrXInd] <- tumor_copy$copy[chrXInd] - chrXMedian
	}
	...
	if (!is.null(normal_panel)){
        ...
        hits <- findOverlaps(tumor_copy, panel, type="equal")
        tumor_copy <- tumor_copy[queryHits(hits),]
        panel <- panel[subjectHits(hits),]
        # subtract out panel median
		tumor_copy$copy <- tumor_copy$copy - panel$Median
		# if male, then shift chrX by +chrXMedian.MNnorm
		if (gender == "male" && exists("chrXMedian.MNnorm")){
			tumor_copy$copy[chrXInd] <- tumor_copy$copy[chrXInd] + chrXMedian.MNnorm
		}
	}
	return(tumor_copy)
}
...
correctReadCounts <- function(x, chrNormalize = c(1:22), mappability = 0.9, samplesize = 50000,
  ...
  x$valid <- TRUE
  x$valid[x$reads <= 0 | x$gc < 0 | is.infinite(x$reads)| is.nan(x$reads) ] <- FALSE
  x$ideal <- TRUE
  routlier <- 0.01
...
option_list <- list(
	make_option(c("--gcWig"), type = "character", help = "GC wig file for reference genome. Required."),
	make_option(c("--mapWig"), type = "character", default=NULL, help = "Mappabiliy wig file for reference genome. Default: NULL"),
	make_option(c("-f", "--filelist"), type = "character", help = "File containing a list of of normal wig file paths. Required"),
	make_option(c("-o", "--outfile"), type = "character", help = "Output file name. Required"),
	make_option(c("-c", "--centromere"), type="character", default=NULL, help = "Tab delimited file containing centromere locations. Default: NULL"),
	make_option(c("--chrs"), type="character", default="c(1:22)", help = "Specify chromosomes to analyze. Default: c(1:22)"),
	make_option(c("--maleChrXLogRThres"), type="numeric", default=-0.80, help = "ChrX Log ratio threshold to confirm as male gender. Default: 0.80"),
	make_option(c("-e", "--exons.bed"), type = "character", default=NULL, help = "Path to bed file containing exon regions. Default: Null"),
	make_option(c("--method"), type = "character", default="median", help="Median or Mean. Default: median")
)
parseobj <- OptionParser(option_list=option_list)
opt <- parse_args(parseobj)

if (is.null(opt$filelist) == TRUE){
 print_help(parseobj)
	q()
}
...
if (!is.null(centromere)){
	centromere <- read.delim(centromere,header=T,stringsAsFactors=FALSE,sep="\t")
}
...
for (i in 1:length(files)){
	...
	normal_reads <- wigToGRanges(files[i],verbose=FALSE)

	message("Reading GC and mappability files")
	if (is.null(gcWig)) {
	  stop("GC Wig file required")
	} else {
	  gc <- wigToGRanges(gcWig,verbose=FALSE)
	}
	if (is.null(mapWig)) {
	  message("Normalizing without mappability Wig file.")
	  map <- NULL
	} else {
	  map <- wigToGRanges(mapWig,verbose=FALSE)
	}
	...
	### CORRECT tumor DATA FOR GC CONTENT AND MAPPABILITY BIASES ###
...
## output to text file ##
write.table(as.data.frame(normalGR[,"Median"]), file=paste0(outfile, "_", method, ".tsv"), col.names=TRUE, row.names=F, quote=F, sep="\t")
